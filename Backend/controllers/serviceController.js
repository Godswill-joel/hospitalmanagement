...
// Helpers Function 
// the function converts array inputs into a clean array
// when the array is empty or invalid it returns an empty arrays
...
export async function createService(req, res) {
    try {
        ...
        }   // if the file is present it will be uploaded into the services folder in the cloudinary;
...
export async function deleteService(req,res) {
    try {
        ...
        if(!existing) return res.status(404).json({
            success:false,
            messsage: "Service is not found"
        });
        if(existing.imagePublicId) {
            try {
                await deleteFromCloudinary(existing.imagePublicId)
            } catch (err) {
                console.warn("Failed to delete image from the cloudinary:",
                    err?.message || err 
                )
            }
        }
